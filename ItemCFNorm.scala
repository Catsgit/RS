...
class ItemCFNormUtil {
  ...
  def itemCFNormEvaluation(num: Int, k: Int): Unit = {
    val recommendItems = scala.collection.mutable.Set[Int]()
    var hit: Int = 0
    var all1: Int = 0
    var all2: Int = 0
    ...
    var n: Int = 0
    for((userId, movies) <- train) {
      val rank = getItemCFNormRecommendation(userId, num, k);
      for(item <- rank.keySet) {
        recommendItems += item
        ret += math.log(1 + itemPopularity(item))
        n += 1
  ...
  def getItemCFNormRecommendation(userId: Int, n: Int, k: Int): scala.collection.immutable.ListMap[Int, Double] = {
    val rs = itemCFNormRecommend(userId, k)
    val rank = scala.collection.immutable.ListMap(rs.toSeq.sortBy(_._2).reverse: _*).take(n)
    return rank
  }

  def itemCFNormRecommend(userId: Int, k: Int): mutable.HashMap[Int, Double] = {
    val rs = mutable.HashMap[Int, Double]()
    val ru = train(userId)
    for(movieId <- ru) {
      val itemSim = w(movieId).take(k)
  ...
  def itemCFNormSimilarity(): Unit = {
    w.clear()
    val tempW = mutable.HashMap[Int, mutable.HashMap[Int, Double]]()
    val n = mutable.HashMap[Int, Int]()
    val c = mutable.HashMap[Int, mutable.HashMap[Int, Double]]()
    for((userId, movies) <- train) {
      for(movieId <- movies) {
    ...
    for((movieId, otherMovies) <- c) {
      for((otherMovieId, cuv) <- otherMovies) {
        if(tempW.contains(movieId)) {
          tempW(movieId) += (otherMovieId -> c(movieId)(otherMovieId) / math.sqrt(n(movieId) * n(otherMovieId)))
        } else {
          tempW += (movieId -> mutable.HashMap(otherMovieId -> c(movieId)(otherMovieId) / math.sqrt(n(movieId) * n(otherMovieId))))
        }
      }
    }
    for(movieId <- c.keySet) {
      val maxMovieId = tempW(movieId).maxBy(_._2)._1
      for(otherMovieId <- c(movieId).keySet) {
        if(w.contains(movieId)) {
          w(movieId) += (otherMovieId -> tempW(movieId)(otherMovieId) / tempW(movieId)(maxMovieId))
        } else {
          w += (movieId -> scala.collection.immutable.ListMap(otherMovieId -> tempW(movieId)(otherMovieId) / tempW(movieId)(maxMovieId)))
        }
      }
    }
    var x = 1
    for(movieId <- w.keySet) {
      w(movieId) = scala.collection.immutable.ListMap(w(movieId).toSeq.sortBy(_._2).reverse:_*)
      println(x)
      x += 1
    }
...
object ItemCFNorm {
  def main(args: Array[String]): Unit = {
    val i = new ItemCFNormUtil
    val writer = new PrintWriter(new File("/home/hadoop/ItemCFNorm"))
    i.readData
    i.splitData(8, 1)
    i.calItemPopularity()
//    i.itemCFSimilarity()
//    i.itemCFEvaluation(10, 10)
//    println("准确率: " + i.precisionValue + " 召回率: " + i.recallValue + " 覆盖率: " + i.coverageValue + " 流行度: " + i.popularityValue)
//    writer.println(i.precisionValue + ":" + i.recallValue + ":" + i.coverageValue + ":" + i.popularityValue)
    i.itemCFNormSimilarity()
    i.itemCFNormEvaluation(10, 10)
    println("准确率: " + i.precisionValue + " 召回率: " + i.recallValue + " 覆盖率: " + i.coverageValue + " 流行度: " + i.popularityValue)
    writer.println(i.precisionValue + ":" + i.recallValue + ":" + i.coverageValue + ":" + i.popularityValue)
    writer.close()
  }
